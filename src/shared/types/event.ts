/**
 * 개발자 행사 하나. collector(grep-airflow)가 티켓타코·이벤터스에서 개발 행사를, Dev-Event(개발자 행사 모음)에서 해커톤을
...
export interface TechEvent {
  /** 티켓타코는 행사 코드(c9xsstcs), 이벤터스는 eventus-{번호}, Dev-Event 는 dev-event-{주소 해시 12자}. React key이자 featured.json 이 가리키는 값이다. */
  id: string
  title: string
  ...
  /** 판매처 이름(티켓타코·이벤터스·Dev-Event). 이 필드가 생기기 전의 파일은 티켓타코뿐이었다. */
  source: EventSource
}

export type EventSource = '티켓타코' | '이벤터스' | 'Dev-Event'
