...
Azar는 전 세계 유저들을 24/7 연결하고 있으며, 즐거운 연결 경험을 제공하기 위해 세션 기반 추천 시스템^[1](#fn:1)^을 운영하고 있습니다. 이 추천 시스템은 유저의 행동이나 선호도 같은 정보를 바탕으로 맞춤형 추천을 제공하는 방식입니다. 추천 품질을 높이려면 유저의 최신 정보를 실시간 반영하는 것이 중요합니다. 유저의 최신 정보는 서로 다른 시점에 실시간으로 생성된 여러 이벤트로 존재했고, 파편화된 이벤트를 조합하여 추천에 활용할 수 있는 데이터로 변환하는 것이 필요했습니다. 뒤에 설명드리겠지만, 실시간으로 여러 이벤트를 조합하는 규칙이 단순하지 않았기 때문에 새로운 스트림 조인 서비스를 구축하게 되었습니다.
...
![실시간 스트림 조인 파이프라인 구성](https://images.gogumang.com/90fc095d0c/01.png) 그림 1. 실시간 스트림 조인 파이프라인 구성
...
|         플랫폼         |              장점              |                 단점                  |                  적합성                  |
|---------------------|------------------------------|-------------------------------------|---------------------------------------|
| **Spark Streaming** | 배치/스트림 통합, 풍부한 Spark API 생태계 | 마이크로배치 기반으로 인한 지연                   | ❌ 밀리초 단위 지연이 중요한 실시간 처리에 부적합          |
| **Kafka Streams**   | 간단한 API, Kafka와 자연스러운 통합     | 복잡한 시간 처리에 제한적 (Schedule 기반 시간만 지원) | ❌ Event Time 기반 타임아웃 등 정밀 제어의 한계      |
| **Apache Flink**    | 낮은 지연, 정밀한 시간 처리 지원          | 높은 학습 곡선, 초기 진입 장벽                  | ✅ 실시간 처리 요구사항(상태, 시간, 지연 제어 등)에 가장 적합 |
...
그럼에도 Window API는 정형화된 시간 기반 처리로, 불특정한 시점에 도착할 이벤트의 경우를 기다리기 어렵습니다. 보다 세밀한 시간 제어가 가능한 도구를 찾게 되었고 Flink의 **KeyedProcessFunction^[2](#fn:3)^**을 선택하게 되었습니다.

KeyedProcessFunction은 다음과 같은 이점을 제공합니다.

* **Key 기반^[3](#fn:4)^ 이벤트 처리**: KeyedProcessFunction은 이벤트를 키별로 분리하여 처리할 수 있어, 각 키에 대한 상태를 독립적으로 관리할 수 있습니다.
...
특히 세밀한 시간 제어가 가능했던 이유는 KeyedProcessFunction에서만 사용 가능한 `TimerService`^[4](#fn:5)^가 있기 때문입니다.
...
![Keyed Process Function을 이용한 이벤트 조합 로직](https://images.gogumang.com/90fc095d0c/02.png) 그림 2. KeyedProcessFunction을 이용한 이벤트 조합 로직
...
![도착/누락 시나리오별 KeyedProcessFunction의 조합 결과 예시](https://images.gogumang.com/90fc095d0c/03.png) 그림 3. 도착/누락 시나리오별 KeyedProcessFunction의 조합 결과 예시
...
Flink는 애플리케이션 수준에서 상태를 관리하기 때문에 Stateful 합니다. 그래서 단순히 Flink 애플리케이션을 배포하는 것만으론 상태를 안전하게 저장하고 복구할 수 없습니다. 상태 유실 없이 안전하게 저장하고 복구할 수 있는 방법이 필요했고, 이를 위해 Flink에서 제공하는 **Savepoint**를 사용했습니다.

## Savepoint로 상태를 복구

Flink Savepoint는 Trigger된 특정 시점의 State Snapshot으로, Flink 애플리케이션의 State를 저장하고 복구하는 데 사용됩니다. 여기서 State Snapshot에는 Kafka Commit Offset, 타이머 그리고 Flink Operator^[5](#fn:6)^에서 정의한 State가 포함됩니다.
...
![Savepoint 생성 후 복구 완료 시점까지의 만료된 타이머](https://images.gogumang.com/90fc095d0c/04.png) 그림 4. Savepoint 생성 후 복구 완료 시점까지의 만료된 타이머
...
Savepoint를 통해 상태를 유실 없이 안전하게 복구할 수 있게 되었습니다. 이제 실시간 전달을 보장하기 위해, 중단 없는 배포 방법을 찾아야 했습니다. 기본적으로 Flink는 무중단 서비스 배포 지원이 없어 직접 구성해야 했고, 제가 찾은 것은 Blue-Green 배포 전략^[6](#fn:7)^이었습니다.
...
![Blue-Green 배포 과정](https://images.gogumang.com/90fc095d0c/05.png) 그림 5. Blue-Green 배포 과정
...
Kafka Sink의 Exactly Once Semantics^[7](#fn:8)^을 활성화하는 방법은 Flink의 기본 설정을 통해 간단하게 적용할 수 있습니다. 그러나 이 방법은 2 Phase Commit(2PC) 매커니즘을 사용하기 때문에 Checkpoint 주기마다 메시지를 발행하게 되어, 실시간성이 크게 저하되는 문제가 있었습니다. Azar의 AI 추천 시스템은 높은 실시간성을 요구했기에 이 방법은 적합하지 않았습니다.
...
![중복 제거 Flink 애플리케이션 별도 구성](https://images.gogumang.com/90fc095d0c/06.png) 그림 6. 중복 제거(Deduplication, Dedup) Flink 애플리케이션 별도 구성
...
![Redis의 Atomic 연산을 활용한 중복 제거(Exactly Once) 파이프라인](https://images.gogumang.com/90fc095d0c/07.png) 그림 7. Redis의 Atomic 연산을 활용한 중복 제거(Exactly Once) 파이프라인
...
1. [Azar의 세션 기반 추천 시스템](https://hyperconnect.github.io/2024/11/19/azar-recommendation-model.html)에서 Azar의 추천 시스템에 대한 자세한 내용을 확인할 수 있습니다. [↩](#fnref:1)

2. [Apache Flink KeyedProcessFunction](https://nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/datastream/operators/process_function/#the-keyedprocessfunction) [↩](#fnref:3)

3. [Apache Flink KeyBy](https://nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/datastream/operators/overview/#keyby) [↩](#fnref:4)

4. [Apache Flink TimerService](https://nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/datastream/operators/process_function/#timers) [↩](#fnref:5)

5. [Apache Flink Operator](https://nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/datastream/operators/overview/) [↩](#fnref:6)

6. [Apache Flink Blue-Green Deployment YouTube](https://www.youtube.com/watch?v=Hyt3YrtKQAM) [↩](#fnref:7)

7. [Apache Flink Exactly Once Semantics](https://flink.apache.org/2018/02/28/an-overview-of-end-to-end-exactly-once-processing-in-apache-flink-with-apache-kafka-too/) [↩](#fnref:8)