...
![](https://cdn-images-1.medium.com/max/1024/1*lggCjpJMXfwxfuFHGECv1g.png)
...
[2026년 상반기 결산](https://about.daangn.com/company/pr/archive/커뮤니티-상반기-결산-데이터-공개/)에서 당근모임의 월간 활성 사용자(MAU)가 천만을 넘었습니다. 커뮤니티 서비스 전체로도 체류시간과 이용자 간 소통이 크게 늘었고, 카페와 아파트 같은 새 커뮤니티 서비스도 빠르게 성장하고 있습니다.
...
![](https://cdn-images-1.medium.com/max/1024/1*Dw_o4RiRpAvHd4DMFtN2mA.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*nreFHMSsyofcVXGWveKdMA.png)

그래서 서버 사이드 렌더링(Server-Side Rendering, SSR)을 도입했습니다. 하지만 API 응답이 늦어지면 빈 화면이 오래 노출되는 문제는 남았습니다. 이를 줄이기 위해 스트리밍 서버 사이드 렌더링(Streaming Server-Side Rendering, Streaming SSR)으로 옮겨왔습니다. API 호출이 필요 없는 화면 뼈대를 먼저 보내고, API가 필요한 본문은 준비되는 대로 이어 보냅니다. 렌더링에 문제가 생기거나 일정 시간을 넘기면 스트리밍을 중단하고 CSR로 전환하는 안전장치도 함께 두었습니다. 이 흐름은 커뮤니티실 구성원인인 리디아(Lydia)가 쓴 [Streaming SSR 온보딩 글](https://medium.com/daangn/입사-1개월차-프론트엔드-개발자의-자체-기술-온보딩-streaming-ssr편-aabaee57f660)과 로토(Roto)의 [Streaming SSR 발표](https://youtu.be/hPyyFu3lrEg?si=LtpJnW0Ofw4zahlR)에서도 더 자세히 볼 수 있습니다.
...
![](https://cdn-images-1.medium.com/max/1024/1*NclQKWoEqzho-ZEyQy0O3Q.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*A8JYvUpcjWKCAbzNAJE3Hg.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*_Xnndy5sarLk5A7kquooRA.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*Hpfmd5cjSh2Nf99VfjLPlQ.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*nC87DsOBA7N0sRdVGFyzyQ.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*F-lbbzb9k1nGBsjAXBQUtw.png)
...
단순한 구조는 저절로 유지되지 않습니다. 하나의 시스템 안에서도 모듈은 계속 태어나고, 나뉘고, 사라집니다. 새 서비스가 시작되면 모듈 경계가 생깁니다. 이웃과 함께 사고 나누는 [같이사요](https://about.daangn.com/company/pr/archive/당근마켓-이웃과-같이-사고-나누는-동네생활-같이사요-오픈)가 공개되었을 때처럼 제품이 새로 태어나면 시스템 안에도 새로운 책임이 필요해집니다. [같이해요가 반짝모임으로 리뉴얼](https://about.daangn.com/company/pr/archive/당근마켓-같이해요-반짝모임으로-리뉴얼-오픈)된 것처럼 제품의 방향이 바뀌면 이미 있던 코드와 모듈도 다시 정리합니다.
...
![](https://cdn-images-1.medium.com/max/1024/1*l4b1_Y047UaRvhJElbDJ1g.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*HQqB61Iu3Vv_GVeZKfZQ-w.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*GH9Chn9TnKXmkFKR1BdgSA.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*ksG_1ZlvvNSZ-K85gQryiA.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*eQPJHGgrIJV_4fUcTzkMhQ.png)
...
![](https://medium.com/_/stat?event=post.clientViewed&referrerSource=full_rss&postId=090fb4021e20)

*** ** * ** ***

[천만 MAU를 지탱하는 커뮤니티 시스템을 소개해요](https://medium.com/daangn/%EC%B2%9C%EB%A7%8C-mau%EB%A5%BC-%EC%A7%80%ED%83%B1%ED%95%98%EB%8A%94-%EC%BB%A4%EB%AE%A4%EB%8B%88%ED%8B%B0-%EC%8B%9C%EC%8A%A4%ED%85%9C%EC%9D%84-%EC%86%8C%EA%B0%9C%ED%95%B4%EC%9A%94-090fb4021e20) was originally published in [당근 기술 블로그](https://medium.com/daangn) on Medium, where people are continuing the conversation by highlighting and responding to this story.