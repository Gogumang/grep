![쿠버네티스 모자를 쓰고 있는 용감한 쿠키](https://images.gogumang.com/57a746c6af/01.png) 쿠버네티스 모자를 쓰고 있는 용감한 쿠키
...
![포드 별로 IAM 권한을 분리하는 경우와 그렇지 않은 경우](https://images.gogumang.com/57a746c6af/02.png) 포드 별로 IAM 권한을 분리하는 경우와 그렇지 않은 경우
...
![kiam과 kube2iam의 동작 원리](https://images.gogumang.com/57a746c6af/03.png) kiam과 kube2iam의 동작 원리
...
1. EC2 인스턴스 메타데이터 API를 호출할 때 사용되는 주소인 169.254.169.254로 향하는 트래픽을 kube2iam이나 kiam으로 라우팅하도록 iptables DNAT 규칙을 워커 호스트에 추가합니다. 이 작업은 보통 애드온이 자동으로 수행합니다.
   ...
   ![169.254.169.254로 향하는 요청을 가로채는 iptables 규칙](https://images.gogumang.com/57a746c6af/04.png) 169.254.169.254로 향하는 요청을 가로채는 iptables 규칙
2. 포드가 사용하는 서비스 어카운트(ServiceAccount)의 주석(Annotation)에 해당 포드가 사용할 IAM 역할을 명시합니다.

   ![포드가 사용할 IAM 역할을 서비스 어카운트에 명시](https://images.gogumang.com/57a746c6af/05.png) 포드가 사용할 IAM 역할을 서비스 어카운트에 명시
...
![서비스 어카운트의 토큰을 jwt.io에서 decode한 데이터](https://images.gogumang.com/57a746c6af/06.png) 서비스 어카운트의 토큰을 jwt.io에서 decode한 데이터
...
![OIDC의 몇 가지 속성이 추가된 쿠버네티스 토큰](https://images.gogumang.com/57a746c6af/07.png) OIDC의 몇 가지 속성이 추가된 쿠버네티스 토큰
...
![EKS 정보에서 확인할 수 있는 IdP 주소](https://images.gogumang.com/57a746c6af/08.png) EKS 정보에서 확인할 수 있는 IdP 주소
...
![Pod Identity Webhook을 사용하기 위한 워크플로우](https://images.gogumang.com/57a746c6af/09.png) Pod Identity Webhook을 사용하기 위한 워크플로우
...
![deco cookie](https://images.gogumang.com/57a746c6af/10.png)
