...
여러분의 회사는 배포를 어떻게 하고 있나요? 배포가 어렵거나 무섭지는 않나요? 뱅크샐러드는 하루에 1,000번 배포하는 조직이 되기 위하여 배포 과정을 개선하고 새로운 배포 시스템을 구축했습니다. 새로운 배포 시스템의 도입을 통해 현재는 예전보다 하루에 무려 4배나 많은 배포가 일어나고 있습니다. 실제로 1,000번의 배포를 하기에는 아직 먼 여정이 남아있지만, 한 가지 확실한 것은 **뱅크샐러드에서 배포는 더는 무서운 것이 아닙니다.** 우리가 어떻게 이런 조직으로 변화하였는지 소개합니다.

*** ** * ** ***

## Old Process

개선된 배포 시스템을 소개하기 전에 간단히 기존 배포 프로세스를 소개하려 합니다.

### Background

먼저 뱅크샐러드의 기본적인 Tech Stack은 다음과 같습니다. 뱅크샐러드는 Version Control System으로 Git을 사용하며 이를 [GitHub](https://github.com)를 통해 관리합니다. MSA~Microservice Architecture~를 적용하여 비즈니스 도메인별로 여러 서비스를 운영하고 있습니다. 모든 서비스는 [Docker](https://www.docker.com)로 containerized 되고, 모든 Docker image는 [Docker Hub](https://hub.docker.com)를 통해 관리됩니다. 이러한 수십 개의 서비스와 container를 [Kubernetes](https://kubernetes.io) 환경에서 운영하고 있습니다. 또한 개발 조직을 비롯한 전사 커뮤니케이션 도구로써 [Slack](https://slack.com)을 사용하고 있습니다.
...
[![Git-Flow branch diagram](https://blog.banksalad.com/static/5c4381fd1ef754f61012604281e848f2/663f3/gitflow.png "Git-Flow branch diagram")](https://blog.banksalad.com/static/5c4381fd1ef754f61012604281e848f2/26e62/gitflow.png)
...
마지막으로 ChatOps를 사용한 배포는 편리했으나, 몇 가지 한계점이 존재했습니다. 먼저 서비스를 새로 배포하기 위해선 기존 배포 서비스의 소스 코드를 직접 수정했어야 했습니다. 배포 서비스 또한 Git-Flow를 사용했기 때문에 앞서 이야기한 불편함을 다 겪은 뒤 배포가 가능했습니다. 또한 실제 배포 시 `kubectl set image` 명령을 통해 Kubernetes Deployment 중 application container의 image tag만을 변경했기 때문에, 이를 제외한 변경은 개발자가 Kubernetes dashboard에 접속하거나 [kubectl](https://kubernetes.io/docs/reference/kubectl/overview/)과 같은 CLI~Command Line Interface~를 사용하여 직접 수정했어야 했습니다. 이로 인해 설정 변경 사항을 추적하기가 힘들어졌고, 설정 오류 등의 human fault가 많이 발생할 수밖에 없었습니다.
...
[![최근 한 달 간 Pull Request 통계](https://blog.banksalad.com/static/38b8e4191c5f43da6944810f2c244f2e/663f3/numbers-of-pull-requests.png "최근 한 달 간 Pull Request 통계")](https://blog.banksalad.com/static/38b8e4191c5f43da6944810f2c244f2e/5f4ca/numbers-of-pull-requests.png)
...
[![GitHub Actions](https://blog.banksalad.com/static/b46179103b1b56e7517e21e5587037d7/663f3/actions.png "GitHub Actions")](https://blog.banksalad.com/static/b46179103b1b56e7517e21e5587037d7/64756/actions.png)
**Figure 3 \| GitHub Actions**

CD~Continuous Delivery~ Pipeline을 GitHub으로 관리하게 되면서 자연스럽게 CI~Continuous Integration~ Pipeline 또한 GitHub를 사용하게 되었습니다. 앞서 말했듯이 개발 조직 내에서 Pull Request 문화가 매우 활성화되어 있는 데다가, 기존에 travis CI 등으로 분산되어 있던 관리 포인트가 GitHub 하나로 일원화되면서 개발자의 편의성이 더욱 증대되었습니다. 또한 GitHub Enterprise 플랜을 사용하고 있어 travis CI보다 동시에 더 많은 잡을 수행할 수 있어^[\[3\]](https://help.github.com/en/actions/getting-started-with-github-actions/about-github-actions#usage-limits)^ 서비스가 많아져도 서로 blocking 없이 진행할 수 있었습니다.
...
[![GitHub Actions를 활용한 배포 sequence diagram](https://blog.banksalad.com/static/ba7aee4364960d4b3a8287b4f3ea6f56/663f3/mermaid-chart-1.png "GitHub Actions를 활용한 배포 sequence diagram")](https://blog.banksalad.com/static/ba7aee4364960d4b3a8287b4f3ea6f56/5da7e/mermaid-chart-1.png)
...
[![@deploybot status](https://blog.banksalad.com/static/0e28b6111665e5508335b965efe4576a/663f3/status.png "@deploybot status")](https://blog.banksalad.com/static/0e28b6111665e5508335b965efe4576a/b06e5/status.png)
...
[![@deploybot deploy](https://blog.banksalad.com/static/d972d3ddf6502e5f53eb6a60e39116a8/663f3/deploy.png "@deploybot deploy")](https://blog.banksalad.com/static/d972d3ddf6502e5f53eb6a60e39116a8/e05f4/deploy.png)
Figure 6 \| @deploybot deploy

주어진 repository의 sha에 해당하는 커밋을 env에 배포합니다. GitHub commit status checks를 통과한 커밋만 배포할 수 있습니다. 어떠한 커밋이든 status check를 통과했다면 어떤 환경이든 배포할 수 있습니다. 해당 커맨드가 실행되면 deploy 서비스는 GitHub API를 통해 deployment를 생성합니다.
...
[![@deploybot history](https://blog.banksalad.com/static/6ecc8321b2b1dd8fea61ac30a8a19916/663f3/history.png "@deploybot history")](https://blog.banksalad.com/static/6ecc8321b2b1dd8fea61ac30a8a19916/2f33f/history.png)
...
[![@deploybot help](https://blog.banksalad.com/static/844dae155b963353b25d428a33d445d6/663f3/help.png "@deploybot help")](https://blog.banksalad.com/static/844dae155b963353b25d428a33d445d6/db5ee/help.png)
...
[![프로세스 개선 전후 배포 과정](https://blog.banksalad.com/static/c1544846769d0f596111ee6c09499927/663f3/mermaid-chart-2.png "프로세스 개선 전후 배포 과정")](https://blog.banksalad.com/static/c1544846769d0f596111ee6c09499927/5da7e/mermaid-chart-2.png)
...
[:chart:](https://blog.banksalad.com/static/41c3d75afbcdf151c0627442908ab95c/dface/chart.png)
...
뱅크샐러드는 우리나라에서 가장 배포를 두려워하지 않고 비즈니스 요구를 충족시키는 테크 기업 1위를 향해서 달려 나가고 있습니다. 실제로 새로 입사한 개발자의 온보딩 과정에는 입사 첫날 임직원 정보 관리 서비스에 자신의 정보를 추가하는 PR을 열고, 이를 merge 한 뒤 deploybot을 활용하여 직접 배포하는 것이 있습니다. 뱅크샐러드에서 배포는 더는 두려움의 대상이 아닙니다. 앞으로 이런 여정을 기술 블로그, 채용 사이트, 자체 컨퍼런스 등에서 지속적으로 함께 펼쳐나갈 계획입니다. 많은 응원 부탁드립니다.
...
* [Deployments \| GitHub Developer](https://developer.github.com/v3/repos/deployments/)
...
* [Kuberentes rollout](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#complete-deployment)

*** ** * ** ***

더 나은 개발 환경에 관심이 있는 분, 두려움 없는 배포로 세상에 더 많은 가치를 만들고 싶으신 분, 실용적이고 빠른 실행으로 성장하고 싶은 분 모두 💚뱅크샐러드💚에서 함께 이야기해요! 🙂