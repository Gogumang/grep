...
![](https://cdn-images-1.medium.com/max/1024/1*b9zku0Izrt8kvjRPLNbLzw.jpeg)This post is also available in [**Korean**](https://medium.com/coupang-engineering/기계-학습-모델을-활용한-물류-입고-프로세스-최적화-fe4490e44514).
...
> · [Background and challenges](#13ce)
> · [Training a model to predict the number of trucks](#2b55)
> ∘ [Feature extraction](#a118)
> ∘ [Model learning: LightGBM algorithm](#bf13)
> ∘ [Model hyper parameter search: Bayesian optimization](#817c)
> ∘ [Inbound reservation system integrated with the model](#d283)
> ∘ [Trade-off between underprediction and overprediction](#b050)
> · [Result of applying the model](#8b3b)
> · [Future plan](#49a2)
...
![Potential resource waste in the supply process from vendors to Coupang fulfillment centers](https://cdn-images-1.medium.com/max/1024/1*jCJozrpL1x9-mwI1nBAW0A.png)Figure 1. Potential resource waste in the supply process
...
![Major tree-based algorithms’ predictive performance on the validation data set of the inbound requests to Coupang’s fulfillment centers](https://cdn-images-1.medium.com/max/819/1*ukrx0gxBz2S5BwxViVIxuA.png)Figure 2. Major tree-based algorithms' predictive performance on the validation set
...
![How the Coupang’s inbound reservation system and the ML model are integrated](https://cdn-images-1.medium.com/max/1024/1*V41Hm0JNBqHLonDpRSYlrA.png)Figure 3. How the IB reservation system and the model are integrated
...
![Result of applying a ML model which predicts the number of trucks that a vendor needs to supply their products to Coupang fulfillment centers](https://cdn-images-1.medium.com/max/711/1*Ikm7Y1TLNj5sy4VhXbJn1Q.png)Table 1. Result of applying a model which predicts the number of trucks
...
![](https://medium.com/_/stat?event=post.clientViewed&referrerSource=full_rss&postId=2db48bbbc304)

*** ** * ** ***

[Optimizing the inbound process with a machine learning model](https://medium.com/coupang-engineering/optimizing-the-inbound-process-with-a-machine-learning-model-2db48bbbc304) was originally published in [Coupang Engineering Blog](https://medium.com/coupang-engineering) on Medium, where people are continuing the conversation by highlighting and responding to this story.