...
조금 더 범위를 축소해 프론트엔드 개발 관점에서 바라본다면, **Framework Agnostic하다는 것은 React, Vue와 같은 웹 프레임워크 혹은 라이브러리에 상관 없이 의도된 동작이 잘 실행되도록 설계되었다는 것**을 의미합니다.
...
![](/images/886c32e5a2/02.png) query-core 의 package.json

` `[query-core](https://github.com/TanStack/query/blob/main/packages/query-core/package.json)` ` [package.json](https://github.com/TanStack/query/blob/main/packages/query-core/package.json)를 보면 dependencies, devDependencies, peerDependencies가 없어요. 어떤 것에도 의존하지 않는 독립적인 로직이 담겨있음을 알 수 있어요.
![](/images/886c32e5a2/03.png) react-query의 package.json

[react-query](https://github.com/TanStack/query/blob/main/packages/react-query/package.json)[package.json](https://github.com/TanStack/query/blob/main/packages/react-query/package.json)을 보면 @tanstack/query-core라는 패키지가 의존성으로 설치되어 있어요. 이를 통해 react-query는 query-core라는 패키지에 의존하고 있고, 이 패키지가 제공하는 모듈을 이용한 코드가 존재한다는 것을 알 수 있어요.
...
이들을 각각 **코어****와****어댑터**라고 부르는데요,**코어에는** 프레임워크에 종속적이지 않은(**Framework Agnostic**) 핵심 로직들이 있고, **어댑터**에는 React, Vue와 같은 프레임워크에서 사용할 수 있도록 코어의 인터페이스를 이용해 코어의 기능을 해당 프레임워크에서 이용할 수 있도록 통합해주는 로직들이 있어요.
...
![](/images/886c32e5a2/04.svg)

## 계산기 라이브러리의 기존 형태

기존의 React 계산기 라이브러리는 아래와 같은 그림으로 설계되어 있다고 가정합니다.
![](https://static.toss.im/ipd-tcs/toss_core/live/b1bac8d9-2335-4d40-8bb2-595fbf9e4acd/4_(1).jpg)

useCalculator라는 하나의 Hook에 위 그림과 같은 구조로 상태와 API가 구현되어 있어요.
![](https://static.toss.im/ipd-tcs/toss_core/live/da2edb7a-b221-4557-a2f4-e911d0d9a1f4/기존_리액트.svg)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/06b67ebd-e962-40c4-bffc-2280cc417bbc/코어리액트관계.png) ![](https://static.toss.im/ipd-tcs/toss_core/live/5c2be29b-503c-4e84-8fb1-63a4b4b3b43a/변경.svg)

## 최종 구조

![](/images/886c32e5a2/10.jpg)
