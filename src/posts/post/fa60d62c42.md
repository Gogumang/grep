...
![/f5c5c744a0ca1f99f5b661ddd77f4cf3/hello.gif](https://tech.inflab.com/f5c5c744a0ca1f99f5b661ddd77f4cf3/hello.gif) \<기존 코드들을 보면서 디버깅하는 내 모습\>
...
![test pyramid](https://tech.inflab.com/static/0538492d2fdd77791f89ee71bf9c5f10/80e3c/test-pyramid.jpg "test pyramid")
...
![bug more and more](https://tech.inflab.com/static/a367c85086bee89fd0af480fdceb2c30/6fe44/bug-more-and-more.png "bug more and more")
...
![ni code is fuck](https://tech.inflab.com/static/ef66c65cc530c6f0ad59bbd74491af02/b4294/ni-code-is-fuck.jpg "ni code is fuck")

그래서 저희는 그들의 편의를 위해 따로 문서화를 해두지만 그 문서를 **쉽게 방치**합니다. 이것이 문서화의 가장 고질적인 문제점입니다.
...
> "신이시여, 이 프로젝트를 구원해주소서."
![believe.png](https://tech.inflab.com/static/7ce1995567e2a9099ff63acc47ff86e4/37523/believe.png "believe.png") \[데브경수\]- 민간신앙
...
`beforeEach`메소드에서 지원자 상태를 불합격(FAIL) 상태가 아닌 **진행중(IN_PROGRESS)으로 초기화** 를 하였기 때문에 실패된것입니다.
...
저희는 테스트를 추가하거나 수정할때마다 항상 `beforeEach`메소드를 파악해야되고,
...
위 작성된 테스트를 보고 상태 변경에 대한 검증 뿐만 아니라 `validateIsNotCancel`와 `validateIsNotFail`메소드가 호출되었는지 검증을 해서 더 완벽한 테스트라고 생각할 수 있습니다.

하지만 이 테스트는 깨지기 쉬운 테스트이면서 좋은 테스트가 아닙니다.

만약 `validateIsNotCancel`메소드의 네이밍이 변경되거나 `validateIsNotFail`메소드가 삭제가 되면 이 테스트는 깨지게 됩니다.

즉 **구현에 의존적이며 테스트의 목적이 구현에 맞춰져 있는 테스트** 입니다.
...
또 하나 이 코드에 아쉬운 점은 바로 **호출되었는지에 대한 검증**입니다.
...
심지어 `User.create`, `Post.create`, `c.updateLikeCount`메소드의 구현부를 봐야할수도 있습니다.
...
테스트 코드 작성 이유중 `세번째 이유. 더 나은 문서 자료`에 대해서 이야기를 한적이 있습니다.
