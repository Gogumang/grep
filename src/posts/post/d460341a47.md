...
![](https://static.toss.im/ipd-tcs/toss_core/live/e1963c2a-7f6c-449e-941f-fd59797eb8b4/image.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/cd9028d0-b768-4b86-af67-3d7da1e4d446/image.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/4552d8cc-6744-45f8-8b21-bc0da9fd13a9/image.png)
...
호출하는 소재가 없는 API는 구현하지 않되, 미지원 호출이 들어와도 크래시 대신 `supports()` false와 `error` 이벤트로 답하게 했어요. [Google 모바일 광고 SDK](https://developers.google.com/ad-manager/mobile-ads-sdk/ios/mraid)도 `getLocation()`은 항상 -1을 반환하고 `storePicture`는 제거했다고 명시할 만큼, 부분 구현은 업계의 일반적인 선택이에요.
...
![](https://static.toss.im/ipd-tcs/toss_core/live/a958ecfb-1964-4d4e-853f-2a8610e2a639/image.png)
...
본체에서 중요했던 결정은 상태 소유권이에요. 실제로 오가는 상태는 `loading` → `default` → `hidden`인데, mraid.js는 네이티브가 준 값을 검증 없이 반영하고 이벤트만 발사해요. 광고 뷰가 화면에 있는지, 사용자가 백그라운드로 나갔는지는 네이티브만 알기 때문이에요. 그림 2의 `ready`가 이 원칙의 출발점이에요. 네이티브가 초기 상태를 한 번에 밀어 넣으면, mraid.js는 상태를 default로 바꾸는 `stateChange`부터 준비 완료를 알리는 `ready`, 노출 상태를 알리는 `viewableChange`까지 연쇄로 발사해요.
...
![](https://static.toss.im/ipd-tcs/toss_core/live/1d03077e-46c6-4b23-8afc-f4337df848d6/image.png)

이 통로를 표준 API인 `window.mraid`에 끼워 넣어도 될까요? 저희는 끼워 넣지 않았어요. mraid.js는 주입될 때 전역 객체를 둘 설치해요. 표준 API를 구현한 `window.mraid`, 그리고 표준 밖 기능을 담당하는 `window.tossPlayableAds`예요.

```javascript
// mraid.js 진입점: 표준과 확장을 별도 전역으로 나눠 설치한다 (게시용으로 축약)
window.mraid = new MraidSDK();      // MRAID 표준 API
window.tossPlayableAds = new TossPlayableAdsSDK();  // 토스 지면 전용 확장 API
```

표준을 지키는 쪽과 넘어서는 쪽을 분리해 두면, 어느 한쪽을 바꿔도 다른 쪽이 흔들리지 않아요.

![](https://static.toss.im/ipd-tcs/toss_core/live/6940f5f4-29ce-4e4b-8165-5e92ca0bac80/image.png)

엔드카드를 쓰려는 소재는 플레이어블 광고가 종료되는 순간 `tossPlayableAds`를 호출하고, 요청을 받은 네이티브가 엔드카드 UI를 노출해요.
...
![](https://static.toss.im/ipd-tcs/toss_core/live/3bcdca8e-e9d1-4e54-b992-bd072ff58a04/image.png)

그래서 표준 밖 신호를 하나 만들었어요. 그림 2에서 `ready`와 나란히 달리던 `rendered`, 웹이 "실제로 그려졌다"를 스스로 감지해 네이티브로 보고하는 신호예요.

이 신호는 네이티브가 노출 판정을 시작하는 선행 조건으로 삼도록 설계했어요. 최소 시청 시간을 재는 스킵 타이머도 `rendered`를 받은 뒤에야 시작해요. 판정이 이르면 빈 화면을 보던 시간이 시청 시간에 포함되고, 늦으면 이미 게임 중인 사용자가 광고를 닫지 못해요.
...
> -   브라우저 첫 페인트(`PerformanceObserver`의 paint 엔트리)는 게임 첫 프레임이 그려지기 전에도 발생할 수 있어요.
> -   소재가 직접 알려 주는 방식은 모든 소재가 약속을 지켜 줘야만 성립해요. 약속을 모르는 소재가 언제든 들어와요.
> -   MRAID 3.0의 `exposureChange`는 광고 컨테이너가 화면에 노출된 비율만 알려 줘요. 컨테이너 안이 실제로 그려졌는지는 답하지 않아요.
...
// 이와 별도로 2.3초 안전망 타이머가 fireRendered('timeout')을 보장한다
```
...
![](https://static.toss.im/ipd-tcs/toss_core/live/6625750d-09b1-4730-ad0d-3a5f8a054b1d/image.png)

이와 별도로 2.3초 안전망 타이머가 항상 돌아요. 감지가 다 실패해도 `source: 'timeout'`으로 반드시 한 번 발사해요. 감지 로직의 실패가 광고 노출 자체를 막으면 안 되기 때문이에요.

`rendered`는 어떤 경로로 몇 ms 만에 감지됐는지를 `{ source, latencyMs }`에 실어 보내요. 타임아웃 폴백 비율이 올라가면 어느 신호가 약한지 추적할 수 있어요.
...
방어 1. SDK를 얼려서 배포해요. 소재는 여러 네트워크에 동시에 제출되기 때문에, 제작사가 로컬 테스트용 mraid 흉내 객체를 번들에 남겨 두는 일이 드물지 않아요. 그 객체가 `window.mraid`를 덮어쓰면 설치 버튼을 눌러도 아무 일도 일어나지 않고 클릭 로그도 남지 않아요. 화면이 깨지는 고장보다 알아채기 어려운 종류예요.
...
`Object.freeze`는 메서드 교체를 막고, `defineProperty`는 `window.mraid = {}`처럼 통째로 갈아끼우는 경로를 막아요. 물론 `Object.freeze`는 얕아서 작정한 우회까지 막지는 못해요. 여기서 막으려는 건 침입이 아니라 사고이고, 실수로 생기는 덮어쓰기는 전부 이 층에서 일어나요.
...
![](https://static.toss.im/ipd-tcs/toss_core/live/8ab53e76-3330-4d85-8d93-9183a59516ea/image.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/1bc8a50e-391c-4887-92aa-a4ddd11263e4/image.png)
