...
![상품(Products)은 병원의 여러 정보가 필요합니다. 비즈니스 로직에서도, 응답 모델 조립에서도요.](https://images.gogumang.com/5ea10ede6d/01.webp)
...
![상품 API 가 병원 서비스를 의존하고 있던 기존 아키텍처는, 병원 서비스의 장애가 전파되는 취약점을 가지고 있었습니다.](https://images.gogumang.com/5ea10ede6d/02.webp)
...
![캐시는 일반적으로 성능 향상을 위해 원본 데이터의 복사본을 일시적으로 보관하는 계층입니다. 캐시 미스가 발생하면 대개 원본 서비스에서 데이터를 조회해 응답하고, 그 결과를 캐시에 적재합니다.](https://images.gogumang.com/5ea10ede6d/03.webp)
...
![상품의 데이터베이스에 병원 데이터의 복제본을 저장합니다. 이는 병원 도메인 변경 이벤트를 수신받으면 갱신되는 “결과적 일관성”을 유지하면서, 장애 전파를 막을 수 있습니다.](https://images.gogumang.com/5ea10ede6d/04.webp)
...
![즉시 일관성은 변경 내용(Writing)을 즉시 읽을(Reading) 수 있습니다. 결과적 일관성에서는 변경 내용을 읽는데 지연이 발생합니다.](https://images.gogumang.com/5ea10ede6d/05.webp)
...
![데이터가 갱신되었지만, 복제본에 전파되지 않은 기간 동안, SCS 시스템은 이전 데이터를 응답합니다.](https://images.gogumang.com/5ea10ede6d/06.webp)
