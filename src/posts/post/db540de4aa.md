...
![그림 1. 스트레스 테스트를 위한 환경 구성도](https://images.gogumang.com/db540de4aa/01.png)
...
![그림 2. 새 시나리오는 코드를 새로 짜는 게 아니라 각 세팅을 따로 조절해서 조합하도록 확장성 있게 관리한다.](https://images.gogumang.com/db540de4aa/02.png)
...
![그림 3. 리얼 환경의 높은 트래픽으로 지표 지연이 있던 순간의 그래프](https://images.gogumang.com/db540de4aa/03.png)
...
![그림 4. 프레임워크 벤치마크 후 분석 report 예시](https://images.gogumang.com/db540de4aa/04.png)
...
![그림 5. locust master 의 탐지 그래프 : 동일 rps를 지속적으로 트래픽을 흘렸을 때 안정적으로 요청을 처리하지 못하는 상황으로 추가적인 원인 파악이 필요](https://images.gogumang.com/db540de4aa/05.png)
...
![그림 6. 이상 상태로 인지한 시스템 지표 : heap이 점진적으로 늘어남에 따라 GC가 일어나는 시점에 executor queue 처리가 힘들어졌다는 결과를 확인했던 실제 스트레스 테스트 지표](https://images.gogumang.com/db540de4aa/06.png)
...
![그림 7. 용량 때문에 hprof 대신 jmap -histo로 살아있는 객체 클래스별 개수 / 바이트만 가볍게 떠서 확인하였다](https://images.gogumang.com/db540de4aa/07.png)
...
![그림 8. 프레임워크 벤치마크 상황중에 기능 카테고리별 CPU 자원 소비 비중을 차지하는지 확인하고자 하는 용도로 cpu profile을 사용](https://images.gogumang.com/db540de4aa/08.png)
...
![그림 9. CPP > Kotlin 포팅 유튜브 내용 중 전후 서버 비교 장표](https://images.gogumang.com/db540de4aa/09.png)
...
![그림 10. 보고서 예시 : 변경된 변수 및 환경에 대한 명시는 반드시 포함](https://images.gogumang.com/db540de4aa/10.png)
...
* `/run-stress` : locust 마스터에 명령을 보내 트래픽을 흘립니다. 때때로 master가 죽어 있는 경우도 있어 검증을 위해 헬스체크를 진행합니다. 또한 현재 worker가 떠 있는지도 함께 확인합니다. SWARM / STOP / RESET/USER_COUNT 등의 명령어를 두어 트래픽을 쉽게 조절할 수 있습니다.
...
![그림 11. 측정 단계를 LLM이 수행가능한 명령단위로 묶음.](https://images.gogumang.com/db540de4aa/11.png)
...
![그림 12. 캡쳐된 그래프의 결과 예시](https://images.gogumang.com/db540de4aa/12.png)
...
![그림 12. 클로드를 어떻게 하면 잘 쓸 수 있을까에 대한 고민의 결과 완성된 작업 플로우.](https://images.gogumang.com/db540de4aa/13.png)
