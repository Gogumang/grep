안녕하세요. 토스증권 Realtime Data Team 강병수 입니다. 토스증권은 데이터센터 장애 상황에도 유저에게 정상적으로 서비스를 제공하기 위해 대부분의 시스템을 이중화했습니다.
...
본 글인 1부에서는 Kafka 이중화 구성에 대한 개요를 담았습니다. 2부에서는 토스증권에서 채택한 Active-Active 구성에서 중요한 부분인 [양방향 데이터 미러링](https://toss.tech/article/kafka-distribution-2)에 대해 소개하고, 3부에서는 Active-Active 구성을 채택 했을 때 운영적으로 높은 난이도를 요구하는 양방향 [Consumer Group Offset Sync에 대해 설명](https://toss.tech/article/kafka-distribution-3)하는 순서로 이어집니다.
...
![](/images/fbd9e5f1fc/01.png) 그림 1. 유저의 트래픽이 이중화된 클러스터로 분산돼서 들어오면 뒷단은 어떻게 구성해야 할까요?
...
Kafka 이중화 구성은 크게 두 가지 아키텍처가 있습니다. **Active-Active 구성**과 **Stretched Cluster 구성**입니다. 이해를 돕기 위해 그림으로 먼저 볼게요.
...
먼저, **Active-Active 구성**은 독립적으로 구성된 두 개의 클러스터를 하나로 묶어주는 과정에서 운영부담이 크다는 단점이 있습니다.
...
**Stretched Cluster**는 물리적으로 분리된 장비에 하나의 Kafka 클러스터로 구성하기 때문에 추가적인 운영부담없이 이중화가 가능하지만, 평시 성능저하와 Split-brain이 발생할 여지가 있습니다.
...
* 이중화 구성 후 트래픽의 흐름
  ![](/images/fbd9e5f1fc/02.png) 그림 4. Active-Active로 Kafka 이중화 구성을 하면 실시간으로 양방향 데이터 미러링으로 각각 100%를 맞춰줘야 하고, Consumer가 데이터센터를 옮겨갈 경우를 대비해서 Consumer Group Offset Sync를 맞춰야 합니다.
...
이번 글 에서는 Kafka 데이터센터 이중화에 대한 개요를 다뤄보았습니다.
데이터센터 이중화 작업을 설계함에 있어 토스증권의 니즈와 환경에 맞춘 적절한 오픈소스가 전무해 Realtime Data Team에서 직접 개발하게 되었는데요. 현재 본래의 기능 구현과 더불어 Active-Active 구성의 단점이었던 운영상 어려움을 많이 해결했습니다. 나아가서 너무나도 중요한 모니터링 환경도 고도화해서 현재 토스증권 시스템에서 중요한 천 개가 넘는 토픽과 Consumer Group을 안정적으로 운영 중입니다.

토스증권 Apache Kafka 데이터센터 이중화 구성의 실제 구현 내용은 [2부 '데이터 미러링](https://toss.tech/article/kafka-distribution-2)'와 [3부 'Offset Sync'](https://toss.tech/article/kafka-distribution-3)에서 이어집니다. 많은 기대 부탁드립니다. 감사합니다.