...
[영상 보기](https://static.toss.im/ipd-tcs/toss_core/live/f4724240-06a4-4622-99fd-a2cc17e25b22/payments-mcp-demo.mp4)
...
![](https://images.gogumang.com/dcb80a0f55/01.avif) 이미지 출처: <https://norahsakal.com/blog/mcp-vs-api-model-context-protocol-explained/>
...
![](https://images.gogumang.com/dcb80a0f55/02.avif) ![](https://images.gogumang.com/dcb80a0f55/03.avif)
...
![](https://images.gogumang.com/dcb80a0f55/04.svg)
Description
![](https://images.gogumang.com/dcb80a0f55/05.svg)
Response

**documents-by-keywords**

LLM 키워드를 적절하게 추출하여 질의하면, MCP 서버가 문서들을 순회하며 다양한 조건별 점수를 계산하고 상위 10개의 문서를 반환하는 도구입니다.
![](https://images.gogumang.com/dcb80a0f55/06.svg)
내부 동작 코드
![](https://images.gogumang.com/dcb80a0f55/07.svg)
Description
![](https://images.gogumang.com/dcb80a0f55/08.svg)
Response

**documents-by-link**

Link 기반으로 문서를 탐색하는 도구입니다. LLM이 문서를 탐색하는 중에 Markdown 링크로 표현된 문서를 만나는 경우 탐색을 실시합니다.
![](https://images.gogumang.com/dcb80a0f55/09.svg)
내부 동작 코드
![](https://images.gogumang.com/dcb80a0f55/10.svg)
Description
![](https://images.gogumang.com/dcb80a0f55/11.svg)
Response
...
![](https://images.gogumang.com/dcb80a0f55/12.svg)
내부 동작 코드
![](https://images.gogumang.com/dcb80a0f55/13.svg)
Description
![](https://images.gogumang.com/dcb80a0f55/14.svg)
Response
...
**문서별 길이가 천차만별이고, 길이가 긴 것은 5,000 라인이 넘어간다는 점**이 원인으로 추측되었어요. 그런 문서들을 LLM이 여러 번 질의하면서 Checkpoint를 만들거나 대화내용을 요약하는 과정에서 중요한 정보들이 유실되는 현상으로 보여졌습니다. 때문에 새로운 방법을 고민해야했고 우리의 친구 Gemini, GPT와 이야기하며 다른 방법을 탐구했어요.

## 두번째 버전

Gemini와 많은 대담을 나누다가 BM25라는 키워드를 확인했고, 해당 방식으로 변경해보기로 했어요.
![](https://images.gogumang.com/dcb80a0f55/15.avif)

**BM25**는 **Best Matching 25**의 약자로, 확률적 정보 검색 모델인 **Okapi BM25**에서 유래했습니다. 이 모델은 특정 문서가 주어진 질의에 얼마나 관련이 있는지를 측정하는 방식으로 작동합니다.

#### 기본 개념

* 문서에 등장한 **질의어(Query term)의 빈도**가 높을수록 관련성이 높다고 판단
* 하지만 **너무 자주 등장하는 단어는 가중치를 낮춤(IDF: 역문서 빈도)**
* 문서 길이에 따라 빈도의 영향을 **보정**함
...
![](https://images.gogumang.com/dcb80a0f55/16.svg)
**GPT표 BM25**
...
* 의미 단위로 잘게 나눠지지 않음
* 조사(는, 에), 어미(는, 다) 등이 그대로 포함되어 **중복되고 불필요한 토큰**이 많아짐
* 같은 의미의 문장도 표현에 따라 일치하지 않음

한국어 문장을 일관성 있고 의미 있게 토큰화하기 위해서는 **형태소 분석 라이브러리**가 필요했어요. 그러나 형태소 분석 라이브러리는 Java나 C와 같은 추가적인 환경 설정을 요구하기 때문에, 이러한 외부 의존성들이 로컬 MCP의 사용성을 저하시킬 수 있다고 판단하여 배제하였습니다. 다행히도 이 문제는 보다 간단한 방식으로 해결이 가능했어요.

MCP 실행 환경에서는 **모든 선택의 주체가 LLM**입니다. 어떤 도구를 사용할지, 그리고 해당 도구를 어떤 파라미터로 실행할지는 전부 LLM이 결정하며, 사용자는 오직 도구 호출 여부만 선택하면 됩니다. 사용자의 질의를 LLM을 통해 유의미한 형태로 토큰화하고, 이 토큰화 된 질의에 정규식을 적용하여 매번 BM25 점수를 계산하면, 질문과 유사한 문서들을 효과적으로 색인할 수 있습니다.
...
![](https://images.gogumang.com/dcb80a0f55/17.avif)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/49f74e76-6ee0-4467-9006-d6150d9b51f1/payments-mcp-1(1).png)
...
**get-v2-documents**
![](https://images.gogumang.com/dcb80a0f55/18.svg)
Description
![](https://images.gogumang.com/dcb80a0f55/19.svg)
Response

**get-v1-documents**
![](https://images.gogumang.com/dcb80a0f55/20.svg)
Description
![](https://images.gogumang.com/dcb80a0f55/21.svg)
Response
...
![](https://images.gogumang.com/dcb80a0f55/22.svg)
Description
![](https://images.gogumang.com/dcb80a0f55/23.svg)
Response

### 구현 후기

이번 구현에서는 다음과 같은 장단점을 얻을 수 있었어요.
* 질의를 바탕으로 유사한 청크들을 조회하므로, **한 번의 질의만으로도 관련된 정보를 포괄적으로 검색**할 수 있게 되었습니다.
* 벡터 DB만큼 정밀하진 않지만, **외부 의존성 없이 로컬 환경에서 RAG 시스템을 구현**할 수 있다는 점에서 유용했습니다.
* 질의와 유사한 정보만을 대상으로 하기 때문에, **불필요한 정보가 줄어들고 Hallucination이 감소**하는 효과를 확인할 수 있었습니다.
* 첫 번째 방식은 각 문서를 Lazy하게 접근하기 때문에 **초기 로딩 비용과 메모리 사용량이 적은 장점**이 있었던 반면, 두 번째 방식은 llms.txt 파일에 포함된 **모든 문서를 시작 시점에 불러오므로 메모리 소비와 부트스트랩 비용이 증가**한다는 단점이 있었습니다.
