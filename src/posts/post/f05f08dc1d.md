#### 엔지니어링 노트 1: 복잡한 퍼널 쉽게 관리하기 엔지니어링 노트 시리즈는 토스페이먼츠 개발자들이 제품을 개발하면서 겪은 기술적 문제와 해결 방법을 직접 다룹니다. 첫 번째는 프론트엔드 이야기인데요. 웹에서 퍼널을 손쉽게 관리할 수 있도록 만든 모듈을 소개합니다.
...
![](https://images.gogumang.com/f05f08dc1d/01.avif)
...
![](https://images.gogumang.com/f05f08dc1d/02.avif)
      ...
      return <PayForm
        onSubmit={() => {
          if (escrowConfig.isAvailable) {
            await startEscrowInputFlow();
          }
          if (isSmsAuthRequired) {
            await startSMSFlow();
          }
          if (isPINRequired) {
            await startPaymentPasswordFlow();
          }
          await confirm();
        }}
      />;
...
![](https://images.gogumang.com/f05f08dc1d/03.avif)
