...
![](https://static.toss.im/ipd-tcs/toss_core/live/cee1cbf3-b162-46b4-a21c-463038704d34/스크린샷_2026-08-24_오전_9.21.23.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/4425442c-8524-437c-a4e6-4106ccd69cac/스크린샷_2026-08-24_오전_9.21.51.png)

**1️⃣ 유저의 관심사가 훨씬 빠르게 변해요**
사용자는 앱 안에서 여러 화면을 탐색하고, 특정 종목을 보고, 뉴스를 읽고, 커뮤니티 글을 봅니다. 그 순간마다 관심 맥락이 계속 달라져요.

2️⃣**시장과 종목 이벤트가 계속 움직여요**
금융 서비스에서는 같은 사용자라도 장중에 관심을 가져야 하는 주제가 계속 변할 수 있습니다.

**3️⃣ 콘텐츠 자체가 다양해졌어요**
뉴스, 커뮤니티, AI 콘텐츠 등 사용자가 탐색할 수 있는 콘텐츠가 계속 늘어나고 있어요.

**4️⃣ 기술적인 조건도 바뀌었어요**
토스증권의 실시간 데이터·머신러닝 플랫폼이 성장하면서, 예전에는 비용 때문에 시도하기 어려웠던 고도화된 추천 시스템을 이제는 구축해서 운영해볼 수 있는 환경이 만들어졌습니다.
...
![](https://static.toss.im/ipd-tcs/toss_core/live/7b31038d-19c8-4c48-b3cb-acf753fb5942/스크린샷_2026-08-24_오전_9.21.44.png)

흐름을 조금 더 자세히 보면,
1. 유저의 행동·액션 이벤트를 받아 저장하고 모델을 학습해요.
2. 새로운 아이템이 생기거나 기존 아이템이 바뀌면 아이템 표현을 다시 만들어 벡터를 인덱싱해요.
3. 추천 요청이 들어오는 온라인 서빙 시점에는, 학습된 리트리버가 벡터 인덱스에서 후보를 찾고 랭커가 점수를 매겨 최종 Top-N을 노출해요.

필연적으로 이런 시스템에는 관측과 운영·디버깅 도구도 필요해집니다. 학습 데이터가 잘 만들어지고 있는지, 인덱스가 잘 갱신되고 있는지, 왜 특정 결과가 추천되었는지 확인할 수 있어야 하니까요.
![](https://static.toss.im/ipd-tcs/toss_core/live/f6964cb4-9928-4a25-9a47-a39fa59e2fa9/스크린샷_2026-08-24_오전_9.22.03.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/df0dc75d-f949-4ddb-9386-7315fac68058/스크린샷_2026-08-24_오전_9.22.21.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/441b6621-c3f4-44cc-b2f4-9afcf9d9f823/스크린샷_2026-08-24_오전_9.28.53.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/ed508e0b-37b1-4489-ae13-6b14eaf71c9c/스크린샷_2026-08-24_오전_9.28.35.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/02d3fdc8-0581-4f9d-bae1-9df18dd9e41a/스크린샷_2026-08-24_오전_9.28.27.png)
...
**벡터 검색은 키워드 중심 질의에서 한계가 있었어요**
금융 특화 임베딩 모델을 학습했음에도, 모든 회사·제품·금융 전문 용어에 대해 학습이 고루 일어나기는 굉장히 어렵습니다. 특히 사용자가 전통적인 키워드 검색 쿼리를 던진 경우에 이 문제가 더 잘 드러났어요.

**텍스트 검색은 사전 관리가 중요했어요**
형태소 분석기가 아직 인식하지 못하는 새로운 금융 이벤트 관련 키워드들이 있었습니다. 이런 경우에는 사전을 업데이트하는 방식의 운영도 필요했어요.

**리랭커로 마지막 노이즈를 줄였어요**
의도가 확실한 질문이 들어왔는데 불필요한 뉴스가 한두 건 섞여 반환되면, 사용자는 "그럴 수 있지"라고 이해하기보다 시스템 자체의 신뢰도를 의심하게 됩니다. 이 부분은 LLM을 파인튜닝한 리랭커를 두면서 해결할 수 있었어요.

**메타데이터는 또 하나의 검색 신호가 되었어요**
뉴스 카테고리를 잘 분류하는 모델을 만들고, 유저 쿼리가 들어오면 함께 분류한 뒤 태그가 매칭되면 더 높은 점수를 주었습니다. 이걸로 검색 정확도를 높일 수 있었어요. 그리고 잘 구축된 분류 체계는 RAG 말고도 여러 제품에서 활용할 수 있었습니다.
...
![](https://static.toss.im/ipd-tcs/toss_core/live/01e2d163-4557-476c-b9b7-a7be40b0504b/스크린샷_2026-08-24_오전_9.30.36.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/024a35a8-177f-4dc4-be02-49291ce0a4ee/스크린샷_2026-08-24_오전_9.30.46.png)
...
따라서 엔티티 추출이나 링킹 과정은 다루지 않습니다. 이 글의 범위는 **사용자 질문과 관련된 서브그래프를 찾아 반환하는 검색 모듈**입니다.
...
이때 모든 관계를 다 따라갈 수는 없어요. 어떤 규칙으로 탐색할 것인지 정해야 하고, **이 규칙을 만들어내는 일이 검색 알고리즘의 핵심**이 됩니다.
![](https://static.toss.im/ipd-tcs/toss_core/live/b8ad6df6-338e-4944-9c63-318a401a470e/스크린샷_2026-08-24_오전_9.31.12.png)
...
![](https://static.toss.im/ipd-tcs/toss_core/live/34cb1297-cd4e-4930-b8ff-755e27cb6490/스크린샷_2026-08-24_오전_9.38.29.png)
...
**어떤 경로가 사용자 질문과 더 관련 있는지**를 점수로 표현해야 했습니다. 질문과의 관련도를 중심에 두고, 금융 도메인에서 중요하게 보는 신호를 함께 반영했어요.
...
PROFILE에서는 인덱스 사용 여부뿐 아니라 예상 행 수와 실제 Rows의 차이, 관계 확장 지점의 DB Hits, 연산자별 메모리 사용량을 확인했습니다. 결국 Cypher 최적화는 **후보가 어느 지점에서 늘어나는지 찾고, 다음 탐색 전에 줄이는 것**이었습니다.
![](/images/4befd0cc35/13.png)

## 그래프 검색의 품질을 결정한 것

그래프 검색의 품질은 그래프를 얼마나 깊게 탐색하느냐보다, **어디서 시작해 어떤 경로를 남기느냐**에 달려 있었습니다.
