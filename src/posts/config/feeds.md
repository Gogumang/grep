...
표의 열은 순서가 고정이다: **블로그 이름 | 피드 주소 | 홈페이지 주소 | 수집**.
홈페이지 주소는 비워둬도 되고, 비우면 피드 주소의 도메인을 대신 쓴다.
수집 칸은 `비활성`일 때만 수집에서 빠진다 — 비어 있거나 칸이 없으면 수집한다.
잠시 멈출 블로그는 행을 지우지 말고 이 칸을 `비활성`으로 둔다. 행을 지우면 다시 넣을 때
피드 주소로 키를 새로 만들어서, 그 블로그로 이미 모아둔 글들과 연결이 끊긴다.

| 블로그 이름 | 피드 주소 | 홈페이지 주소 | 수집 |
| --- | --- | --- | --- |
